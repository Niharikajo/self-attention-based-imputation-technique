...
from Data_Preprocessing.data_processing_utils import window_truncate, random_mask, add_artificial_mask, \
    saving_into_h5
...
if __name__ == '__main__':
    ...
    logger = setup_logger(os.path.join(dataset_saving_dir + "/dataset_generating.log"),
                          'Generate Battery dataset', mode='w')
    logger.info(args)

    df = pd.read_csv(args.file_path, sep=',', decimal='.')
    ...
    selected_as_test = unique_days[:2] 
    logger.info(f'days selected as test set are {selected_as_test}')
    selected_as_val = unique_days[2:3]
    logger.info(f'days selected as val set are {selected_as_val}')
    selected_as_train = unique_days[3:]
    logger.info(f'days selected as train set are {selected_as_train}')
    ...
    feature_names=['Charge_Capacity(Ah)','Voltage(V)']
